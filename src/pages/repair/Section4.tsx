...
const Section4 = ({subDomain, handleStatus}: Props) => {
  ...
  return (
    <section className={subDomain + '-Container'}>
      <Grid container className={classes.root} spacing={2} style={{display: subDomain === 'mobiletechlab' ? 'none' : 'flex'}}>
        <Grid item xs={12} md={6} className={classes.item1}>
          ...
          <Box className={subDomain + '-repair-section-button'}>
            <Link to='#' style={{textDecoration: 'none'}} onClick={()=>{handleStatus(true)}}>
              <Button
