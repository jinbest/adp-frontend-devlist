    ...
    <section className={subDomain + '-Container'}>
      <Grid item xs={12} sm={12} className={subDomain + '-section1-top'}>
        <div className={subDomain + "-section1-title"}>
          {subDomain === 'mobiletechlab' ? t(data.homeTextData.section1.title[0]) : customeTitle + ' ' + t(data.homeTextData.section1.title[0])}
        </div>
        <div className={subDomain + "-section1-title"}>
          ...
          return (            
            <FeatureToggles features={feats} key={index}>
              <Feature
                ...
                inactiveComponent={()=><></>}
                activeComponent={()=>
                  <Grid item xs={6} sm={6} md={gridMD} style={{paddingTop: '0px', margin: '0 auto 5px', maxWidth: '500px'}}>
                    <CardMobile 
                      title={t(item.title)} 
                      img={item.img} 
